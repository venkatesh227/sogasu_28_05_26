...
// ===== FETCH ORDER DATA =====
$stmt = $pdo->prepare("
    SELECT o.*, c.first_name, c.last_name, cat.category_name, sc.name as sub_category_name
    FROM orders o
...
// ===== FETCH EMPLOYEES =====
$employees = $pdo->query("SELECT id, first_name, last_name, job_role FROM employees WHERE status=1 AND is_deleted=0 ORDER BY first_name ASC")->fetchAll();
...
<main class="main-content">
    ...
    <div >
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <div>
                 <h2 style="font-size: 1.5rem; font-weight: 700; color: #1e293b;">Edit Order #<?= htmlspecialchars($order['order_code']) ?></h2>
                 <p class="text-muted">Reassign employee or update status</p>
            </div>
            <a href="orders.php" class="btn" style="background: white; border: 1px solid #e2e8f0; color: #64748b; text-decoration: none; padding: 0.5rem 1rem; border-radius: 6px; display: flex; align-items: center; gap: 0.5rem;">
    ...
    <?php if (isset($error)): ?>
        <div style="background: #fee2e2; color: #dc2626; padding: 1rem; border-radius: 8px; margin-bottom: 1rem; border: 1px solid #fecaca;">
            <?= $error ?>
        </div>
    <?php endif; ?>
    ...
    <form method="POST" style="display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem;">
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="<?= htmlspecialchars($order['first_name'] . ' ' . $order['last_name']) ?>" readonly style="background: #f8fafc;">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Garment Type</label>
                        <input type="text" class="form-control" value="<?= htmlspecialchars($order['category_name'] . ' - ' . $order['sub_category_name']) ?>" readonly style="background: #f8fafc;">
                    </div>
                </div>

                <div class="form-group" style="margin-top: 1.5rem;">
                    <label class="form-label">Design Notes / Instructions</label>
                    <textarea name="notes" class="form-control" rows="5" placeholder="Enter stitching instructions..."><?= htmlspecialchars($order['notes']) ?></textarea>
                </div>
            </div>
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
                    <div class="form-group">
                        ...
                        <div style="font-weight: 700; font-size: 1.1rem;">₹ <?= number_format($order['base_price'], 2) ?></div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Extra Charges</label>
                        <div style="font-weight: 700; font-size: 1.1rem;">₹ <?= number_format($order['extra_charges'], 2) ?></div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Total Amount</label>
                        <div style="font-weight: 800; font-size: 1.2rem; color: var(--primary);">₹ <?= number_format($order['total_amount'], 2) ?></div>
                    </div>
                </div>

                <?php if (!empty($order_services)): ?>
                    <div style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px dashed #e2e8f0;">
                        <label class="form-label" style="font-size: 0.75rem; text-transform: uppercase; color: #94a3b8;">Included Services</label>
                        <div style="display: flex; flex-wrap: wrap; gap: 0.75rem; margin-top: 0.5rem;">
                            <?php foreach ($order_services as $srv): ?>
                                <div style="background: #f1f5f9; padding: 0.5rem 0.75rem; border-radius: 6px; font-size: 0.85rem; color: #475569; border: 1px solid #e2e8f0;">
                                    <i class="ri-checkbox-circle-line" style="color: #22c55e;"></i> <?= htmlspecialchars($srv['service_name']) ?> (₹<?= number_format($srv['service_price']) ?>)
                                </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 1.5rem;">
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px; border-top: 4px solid var(--primary);">
                <h3 style="font-size: 1.1rem; font-weight: 600; color: #1e293b; margin-bottom: 1.5rem;">Workflow Management</h3>
                
                <div class="form-group" style="margin-bottom: 1.25rem;">
                     <label class="form-label">Assign / Reassign To</label>
                     <p style="font-size: 0.75rem; color: #64748b; margin-top: -0.25rem; margin-bottom: 0.5rem;">Select an employee to handle this order.</p>
                     <select name="assigned_employee_id" class="form-select" style="border-color: #4f46e5; border-width: 2px;">
...
<?php endforeach; ?>
                ...
                <button type="submit" class="btn btn-primary" style="width: 100%; justify-content: center; padding: 1rem; font-weight: 700; font-size: 1rem;">
                    <i class="ri-save-line"></i> Save Changes
                </button>
                
                <a href="orders.php" style="display: block; text-align: center; margin-top: 1rem; font-size: 0.85rem; color: #64748b; text-decoration: none;">
                    Discard and Go Back
                </a>
            </div>
...
<style>
    ...
    .btn-primary { background: #4f46e5; color: white; border: none; border-radius: 8px; display: flex; align-items: center; gap: 0.5rem; cursor: pointer; transition: all 0.2s; }
