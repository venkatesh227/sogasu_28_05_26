...
$activeOrders = $pdo->query("
    SELECT o.id, o.order_code, o.due_date, o.order_status, o.supervisor_id, sc.name as garment
    FROM orders o
...
<main class="main-content">
    ...
    <div >
        ...
        <div style="display: flex; gap: 0.75rem;  overflow-x: auto; ">
            <a href="orders.php?tab=all" class="status-tab" style="background: <?= $tab == 'all' ? '#4f46e5' : '#ffffff' ?>; color: <?= $tab == 'all' ? 'white' : '#64748b' ?>; border: 1px solid <?= $tab == 'all' ? '#4f46e5' : '#e2e8f0' ?>;">All Orders</a>
            <a href="orders.php?tab=pending" class="status-tab" style="background: <?= $tab == 'pending' ? '#4f46e5' : '#ffffff' ?>; color: <?= $tab == 'pending' ? 'white' : '#64748b' ?>; border: 1px solid <?= $tab == 'pending' ? '#4f46e5' : '#e2e8f0' ?>;">Pending</a>
            <a href="orders.php?tab=processing" class="status-tab" style="background: <?= $tab == 'processing' ? '#4f46e5' : '#ffffff' ?>; color: <?= $tab == 'processing' ? 'white' : '#64748b' ?>; border: 1px solid <?= $tab == 'processing' ? '#4f46e5' : '#e2e8f0' ?>;">Processing</a>
            <a href="orders.php?tab=stitching" class="status-tab" style="background: <?= $tab == 'stitching' ? '#4f46e5' : '#ffffff' ?>; color: <?= $tab == 'stitching' ? 'white' : '#64748b' ?>; border: 1px solid <?= $tab == 'stitching' ? '#4f46e5' : '#e2e8f0' ?>;">Stitching</a>
            <a href="orders.php?tab=ready" class="status-tab" style="background: <?= $tab == 'ready' ? '#4f46e5' : '#ffffff' ?>; color: <?= $tab == 'ready' ? 'white' : '#64748b' ?>; border: 1px solid <?= $tab == 'ready' ? '#4f46e5' : '#e2e8f0' ?>;">Ready for Delivery</a>
            <a href="orders.php?tab=delivered" class="status-tab" style="background: <?= $tab == 'delivered' ? '#4f46e5' : '#ffffff' ?>; color: <?= $tab == 'delivered' ? 'white' : '#64748b' ?>; border: 1px solid <?= $tab == 'delivered' ? '#4f46e5' : '#e2e8f0' ?>;">Delivered</a>
        </div>
        ...
        <div class="table-container">
            <table id="ordersTable" class="table">
                ...
                <tbody>
                    ...
                    $query = "
                        SELECT 
                            o.id, o.order_code, o.due_date, o.order_status, o.supervisor_id,
                    ...
                    foreach ($orders_list as $o):
                        switch($o['order_status']) {
                            ...
                            case 'stitching': $statusColor = '#6366f1'; break;
                            case 'ready': $statusColor = '#0891b2'; break;
                    ...
                    <tr style="border-bottom: 1px solid #f1f5f9;">
                        <td style="padding: 1rem; font-weight: 700; color: #4f46e5;">#<?= htmlspecialchars($o['order_code']) ?></td>
                        <td style="padding: 1rem; font-weight: 600; color: #1e293b;"><?= htmlspecialchars($o['first_name'] . ' ' . $o['last_name']) ?></td>
                        <td style="padding: 1rem; color: #64748b;"><?= htmlspecialchars($o['garment'] ?? 'General') ?></td>
                        <td style="padding: 1rem;">
                            <?php $img = !empty($o['fabric_img']) ? '../'.$o['fabric_img'] : 'https://via.placeholder.com/40'; ?>
                            <img src="<?= $img ?>" style="width: 40px; height: 40px; border-radius: 8px; object-fit: cover; border: 1px solid #e2e8f0;">
                        </td>
                        <td style="padding: 1rem;">
                        ...
                        <td style="padding: 1rem; font-weight: 600;">
                            ...
                            ?>
                            <span style="color: <?= $dueColor ?>; background: <?= $dueBg ?>; padding: <?= $dueBg !== 'transparent' ? '4px 8px' : '0' ?>; border-radius: 6px; display: inline-block;">
                                <?= date('d M Y', strtotime($o['due_date'])) ?>
                            </span>
                        </td>
                        <td style="padding: 1rem; text-align: right;">
                            <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
                                <button onclick="openSupervisorModal(<?= $o['id'] ?>, '<?= htmlspecialchars($o['order_code']) ?>', '<?= htmlspecialchars($o['supervisor_id'] ?? '') ?>')" class="btn btn-sm" style="background: #f8fafc; color: #f59e0b; border: 1px solid #e2e8f0; padding: 5px 10px; border-radius: 6px; cursor: pointer;" title="Assign Supervisor"><i class="ri-user-star-line"></i> Supervisor</button>
                                <a href="view-order.php?id=<?= $o['id'] ?>" class="btn btn-sm" style="background: #f8fafc; color: #6366f1; border: 1px solid #e2e8f0; padding: 5px 10px; border-radius: 6px; text-decoration: none;"><i class="ri-eye-line"></i> View</a>
                                <a href="edit-order.php?id=<?= $o['id'] ?>" class="btn btn-sm" style="background: #f8fafc; color: #64748b; border: 1px solid #e2e8f0; padding: 5px 10px; border-radius: 6px; text-decoration: none;"><i class="ri-pencil-line"></i> Edit</a>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
...
<div id="supervisorModal" style="display: none; position: fixed; inset: 0; z-index: 9999; background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(4px); align-items: center; justify-content: center; transition: all 0.3s;">
    <div style="background: white; border: 1px solid #e2e8f0; border-radius: 12px; max-width: 400px; width: 90%; padding: 1.5rem; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04); transform: scale(0.95); transition: all 0.2s;" id="modalContent">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.25rem;">
            ...
            <button onclick="closeSupervisorModal()" style="border: none; background: transparent; color: #64748b; font-size: 1.2rem; cursor: pointer;"><i class="ri-close-line"></i></button>
        </div>
        ...
        <form method="POST">
            ...
            <div style="margin-bottom: 1rem;">
                ...
                <select name="supervisor_id" id="modalSupervisorSelect" style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='#4f46e5'" onblur="this.style.borderColor='#cbd5e1'">
            ...
            <div style="display: flex; gap: 0.75rem; justify-content: flex-end;">
                <button type="button" onclick="closeSupervisorModal()" style="padding: 0.625rem 1rem; border: 1px solid #e2e8f0; background: white; color: #475569; font-weight: 600; border-radius: 8px; cursor: pointer; transition: background 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='white'">Cancel</button>
                <button type="submit" style="padding: 0.625rem 1.25rem; border: none; background: #4f46e5; color: white; font-weight: 600; border-radius: 8px; cursor: pointer; transition: opacity 0.2s;" onmouseover="this.style.opacity='0.9'" onmouseout="this.style.opacity='1'">Assign</button>
            </div>
        </form>
    </div>
</div>
