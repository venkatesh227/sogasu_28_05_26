    ...
    <div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; ">
            <div>
                ...
                <p style="color: #64748b; margin-top: 0.25rem;">Manage staff schedules and approve shift change
                    requests.</p>
            </div>

            <div style="display: flex; gap: 0.75rem; align-items: center;">
                <div
                    style="display: flex; align-items: center; background: white; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; height: 38px;">
                    <a href="?date=<?= $prev_week ?>"
                        style="padding: 0 12px; color: #64748b; text-decoration: none; border-right: 1px solid #e2e8f0; display: flex; align-items: center; height: 100%;"><i
                            class="ri-arrow-left-s-line"></i></a>
                    <div
                        style="font-weight: 600; color: #334155; font-size: 0.85rem; padding: 0 15px; text-align: center; background: #f8fafc; display: flex; align-items: center; height: 100%;">
                        <?= date('d M', strtotime($week_start)) ?> - <?= date('d M Y', strtotime($week_end)) ?>
                    </div>
                    <a href="?date=<?= $next_week ?>"
                        style="padding: 0 12px; color: #64748b; text-decoration: none; border-left: 1px solid #e2e8f0; display: flex; align-items: center; height: 100%;"><i
                            class="ri-arrow-right-s-line"></i></a>
                </div>

                <button onclick="window.location.href='?date=<?= date('Y-m-d') ?>'" class="btn btn-light"
                ...
                <?php if (count($pending_requests) > 0): ?>
                    <button onclick="window.location.href='shift-requests.php'" class="btn"
                        ...
                        <span
                            style="position: absolute; top: -6px; right: -6px; background: #ef4444; color: white; width: 18px; height: 18px; border-radius: 50%; font-size: 0.65rem; display: flex; align-items: center; justify-content: center; font-weight: 700;"><?= count($pending_requests) ?></span>
                    </button>
                <?php endif; ?>

                <button onclick="showShiftTypeManager()" class="btn btn-primary" style="height: 38px;"><i
                        class="ri-settings-4-line"></i> Configure Shifts</button>
            </div>
        </div>

        <!-- Filters & Legend Container -->
        <div class="table-container"
            style="padding: 1.25rem; margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center; gap: 2rem; flex-wrap: wrap;">
            <form method="GET" style="display: flex; gap: 0.75rem; flex: 1; align-items: center;">
                ...
                <div style="position: relative; max-width: 250px; width: 100%;">
                    ...
                    <input type="text" name="employee_name" value="<?= htmlspecialchars($filter_name) ?>"
                        placeholder="Search staff..." class="form-control" style="padding-left: 36px; height: 38px;">
                </div>
                ...
                <?php if ($filter_name || $filter_role): ?>
                    <a href="shift-roster.php?date=<?= $current_date ?>" class="btn btn-light"
                        style="height: 38px; display: flex; align-items: center;">Reset</a>
                <?php endif; ?>
            </form>

            <div
                style="display: flex; gap: 1rem; align-items: center; background: #f8fafc; padding: 0.5rem 1rem; border-radius: 8px; border: 1px solid #e2e8f0;">
                <span
                    style="font-size: 0.75rem; font-weight: 700; color: #64748b; text-transform: uppercase;">Legend:</span>
                <?php foreach ($shift_types as $type): ?>
                    <div style="display: flex; align-items: center; gap: 0.4rem; font-size: 0.8rem; font-weight: 600; color: #334155;"
                        title="<?= date('g:i A', strtotime($type['start_time'])) ?> - <?= date('g:i A', strtotime($type['end_time'])) ?>">
                        <span
                            style="width: 12px; height: 12px; border-radius: 3px; background: <?= $type['color'] ?>;"></span>
                        <?= $type['short_code'] ?>
                    </div>
                <?php endforeach; ?>
            </div>
                ...
                <table class="table" style="width: 100%; border-collapse: collapse; min-width: 1000px; margin: 0;">
                    <thead>
                        <tr style="background: #f8fafc; border-bottom: 1px solid #e2e8f0;">
                            <th
                                style="padding: 1.25rem 1rem; text-align: left; color: #64748b; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; width: 250px; position: sticky; left: 0; background: #f8fafc; z-index: 10; border-right: 1px solid #e2e8f0;">
                                Staff Member</th>
                    ...
                    <tbody>
                        <?php foreach ($employees as $emp):
                            ...
                            <tr style="border-bottom: 1px solid #f1f5f9;">
                                <td
                                    style="padding: 1rem; position: sticky; left: 0; background: white; z-index: 5; border-right: 1px solid #e2e8f0;">
...
<div id="shiftModal"
    ...
    <div
        style="background: white; width: 100%; max-width: 400px; border-radius: 12px; padding: 1.5rem; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            ...
            <button onclick="closeShiftModal()"
                style="background: none; border: none; color: #64748b; cursor: pointer; font-size: 1.25rem;"><i
        ...
        <div style="display: grid; grid-template-columns: 1fr; gap: 0.75rem;">
            <?php foreach ($shift_types as $type): ?>
                <button onclick="updateShift(<?= $type['id'] ?>)"
                    style="padding: 1rem; border: 1px solid #e2e8f0; border-radius: 8px; background: white; cursor: pointer; text-align: left; display: flex; align-items: center; gap: 1rem; transition: border-color 0.2s;">
            ...
            <?php endforeach; ?>
            <button onclick="updateShift(0)"
                style="margin-top: 0.5rem; padding: 1rem; border: 1px dashed #ef4444; border-radius: 8px; background: #fef2f2; color: #ef4444; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
...
<div id="shiftTypeModal"
    style="display: none; position: fixed; inset: 0; background: rgba(0, 0, 0, 0.4); z-index: 1000; align-items: center; justify-content: center;">
    <div
        style="background: white; width: 100%; max-width: 800px; border-radius: 12px; padding: 1.5rem; box-shadow: 0 10px 25px rgba(0,0,0,0.1); display: flex; flex-direction: column; max-height: 90vh;">
        <div
            ...
            <button onclick="closeShiftTypeModal()"
                style="background: none; border: none; color: #64748b; cursor: pointer; font-size: 1.25rem;"><i
                    class="ri-close-line"></i></button>
        </div>
        ...
        <div style="display: flex; gap: 1.5rem; flex: 1; overflow: hidden; min-height: 450px;">
            ...
            <div
                style="width: 220px; border-right: 1px solid #e2e8f0; padding-right: 1rem; display: flex; flex-direction: column; gap: 0.5rem; overflow-y: auto;">
                <?php foreach ($shift_types as $type): ?>
                    <button class="shift-type-tab" id="tab-<?= $type['id'] ?>"
                        onclick="selectShiftType(<?= htmlspecialchars(json_encode($type)) ?>)"
                        style="padding: 0.75rem 1rem; border: 1px solid #e2e8f0; border-radius: 8px; background: white; cursor: pointer; text-align: left; transition: all 0.2s; display: flex; align-items: center; gap: 0.75rem; width: 100%;">
                        <div style="width: 10px; height: 10px; border-radius: 30%; background: <?= $type['color'] ?>;">
                        </div>
                        <div style="flex: 1;">
            ...
            <form id="shiftTypeForm" onsubmit="event.preventDefault(); submitShiftTypeUpdate();"
                style="flex: 1; overflow-y: auto; padding-right: 0.5rem; display: flex; flex-direction: column; gap: 1rem;">
                ...
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem;">
                    <div>
                        <label
                            style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.25rem;">Shift
                            Name</label>
                        <input type="text" name="name" id="edit-shift-name" required class="form-control"
                            style="height: 38px;">
                    </div>
                    <div>
                        <label
                            style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.25rem;">Short
                            Code</label>
                        <input type="text" name="short_code" id="edit-shift-short" class="form-control"
                            style="height: 38px;">
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
                    <div>
                        <label
                            style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.25rem;">Start
                            Time</label>
                        <input type="time" name="start_time" id="edit-shift-start" required class="form-control"
                            style="height: 38px;">
                    </div>
                    <div>
                        <label
                            style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.25rem;">End
                            Time</label>
                        <input type="time" name="end_time" id="edit-shift-end" required class="form-control"
                            style="height: 38px;">
                    </div>
                    <div>
                        <label
                            style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.25rem;">Color
                            (Hex)</label>
                        <div style="display: flex; gap: 0.25rem;">
                            <input type="color" id="edit-shift-color-picker"
                                oninput="document.getElementById('edit-shift-color').value = this.value"
                                style="width: 38px; height: 38px; border: 1px solid #e2e8f0; border-radius: 6px; padding: 2px; cursor: pointer; background: white;">
                            <input type="text" name="color" id="edit-shift-color"
                                oninput="document.getElementById('edit-shift-color-picker').value = this.value" required
                                class="form-control" style="height: 38px; flex: 1;">
                        </div>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem;">
                    <div>
                        <label
                            style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.25rem;">Auto
                            Clock Out (Hours)</label>
                        <input type="number" name="auto_clock_out" id="edit-shift-auto-out" class="form-control"
                            style="height: 38px;">
                    </div>
                    <div>
                        <label
                            style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.25rem;">Half
                            Day Mark Time</label>
                        <input type="time" name="half_day_mark_time" id="edit-shift-half-time" class="form-control"
                            style="height: 38px;">
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
                    <div>
                        <label
                            style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.25rem;">Early
                            Clock In (Min)</label>
                        <input type="number" name="early_clock_in" id="edit-shift-early-in" class="form-control"
                            style="height: 38px;">
                    </div>
                    <div>
                        <label
                            style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.25rem;">Late
                            Mark After (Min)</label>
                        <input type="number" name="late_mark_after" id="edit-shift-late-after" class="form-control"
                            style="height: 38px;">
                    </div>
                    <div>
                        <label
                            style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.25rem;">Max
                            Checkin Count</label>
                        <input type="number" name="max_checkin" id="edit-shift-max-check" class="form-control"
                            style="height: 38px;">
                    </div>
                </div>
                ...
                <div>
                    <label
                        style="display: block; font-size: 0.75rem; font-weight: 600; color: #64748b; margin-bottom: 0.5rem;">Office
                        Opens On</label>
                    <div style="display: flex; flex-wrap: wrap; gap: 0.75rem;">
                        ...
                        foreach ($weekdays as $day):
                            ?>
                            <label
                                style="display: flex; align-items: center; gap: 0.35rem; font-size: 0.8rem; color: #334155; cursor: pointer;">
                                <input type="checkbox" name="office_days[]" value="<?= $day ?>" class="office-day-checkbox"
                ...
                <div
                    style="margin-top: auto; display: flex; justify-content: flex-end; gap: 0.75rem; border-top: 1px solid #e2e8f0; padding-top: 1rem;">
