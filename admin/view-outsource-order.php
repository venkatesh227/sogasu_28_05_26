...
// Outsource order images from outsource_orders table columns
if (!empty($order['material_image'])) {
    $images[] = [
        'image_path' => $order['material_image'],
        'image_type' => 'fabric'
    ];
}

if (!empty($order['referral_image'])) {
    $images[] = [
        'image_path' => $order['referral_image'],
        'image_type' => 'sample'
    ];
}
...
$fabric_images = array_filter($images, function ($img) {
    return isset($img['image_type']) && $img['image_type'] === 'fabric';
});

$sample_images = array_filter($images, function ($img) {
    return isset($img['image_type']) && $img['image_type'] === 'sample';
});
    ...
    <div>
        <div style="display: flex; align-items: center; justify-content: space-between;">
            ...
            <div style="display: flex; gap: 1rem;">
                <button class="btn" onclick="window.location.href='outsourcing_orders.php'"
            ...
            <div style="background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 1.5rem;">
                ...
                <div>
                    ...
                    <div style="display: flex; align-items: center; gap: 0.5rem; color: #1e293b; font-weight: 600;">
                        ...
                        <?= !empty($order['rack_name']) ? $order['rack_name'] : '<span style="color:#94a3b8; font-style:italic; font-weight:400;">Not Allocated</span>' ?>
                    </div>
                </div>
            </div>
