...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['certificate'])) {
    ...
    if (move_uploaded_file($file['tmp_name'], $target_file)) {
        ...
        header("Location: view-employee.php?id=$id&upload=success");
        exit;
    }
}
...
if (isset($_GET['delete_cert'])) {
    ...
    if ($cert) {
        ...
        header("Location: view-employee.php?id=$id&delete=success");
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_password'])) {
    ...
    header("Location: view-employee.php?id=$id&reset=success");
    exit;
}
...
$otStmt = $pdo->prepare("SELECT * FROM employee_overtime WHERE employee_id = ? ORDER BY ot_date DESC");
$otStmt->execute([$id]);
$ot_logs = $otStmt->fetchAll();
    ...
    <div >
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <div style="display: flex; align-items: center; gap: 1rem;">
                <a href="employees.php" class="btn-icon" style="text-decoration: none;"><i class="ri-arrow-left-line"></i></a>
                <div>
        ...
        <div style="display: flex; flex-direction: column; gap: 2rem;">
            ...
            <div style="background: white; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
                <div style="padding: 1.25rem 1.5rem; border-bottom: 1px solid #f1f5f9; background: #fff;">
                    <h3 style="font-size: 1.1rem; font-weight: 700; color: #1e293b; margin: 0; display: flex; align-items: center; gap: 0.5rem;">
                        <i class="ri-timer-flash-line" style="color: #059669;"></i> Overtime History
                    </h3>
                </div>
                <div style="padding: 0;">
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background: #f8fafc; text-align: left; color: #64748b; font-size: 0.75rem; font-weight: 700; text-transform: uppercase;">
                                ...
                                <th style="padding: 1rem 1.5rem;">Hours</th>
                                <th style="padding: 1rem 1.5rem;">Amount</th>
                                <th style="padding: 1rem 1.5rem;">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (empty($ot_logs)): ?>
                                <tr>
                                    <td colspan="4" style="padding: 2rem; text-align: center; color: #94a3b8; font-size: 0.9rem;">No OT logs found for this employee.</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($ot_logs as $log): 
                                    $statusColor = $log['status'] == 'Approved' ? '#059669' : ($log['status'] == 'Pending' ? '#d97706' : '#e11d48');
                                    $statusBg = $log['status'] == 'Approved' ? '#dcfce7' : ($log['status'] == 'Pending' ? '#fffbeb' : '#fff1f2');
                                ?>
                                    <tr style="border-bottom: 1px solid #f1f5f9;">
                                        <td style="padding: 1rem 1.5rem;">
                                            <div style="font-weight: 600; color: #1e293b;"><?= date('d M, Y', strtotime($log['ot_date'])) ?></div>
                                            <div style="font-size: 0.75rem; color: #94a3b8;"><?= htmlspecialchars($log['description']) ?></div>
                                        </td>
                                        <td style="padding: 1rem 1.5rem;">
                                            <div style="font-weight: 700; color: #1e293b;"><?= number_format($log['hours'], 1) ?> <span style="font-size: 0.75rem; font-weight: 400;">Hrs</span></div>
                                            <div style="font-size: 0.7rem; color: #94a3b8;">Rate: ₹<?= number_format($log['hourly_rate'], 0) ?></div>
                                        </td>
                                        <td style="padding: 1rem 1.5rem;">
                                            <div style="font-weight: 700; color: #059669;">₹<?= number_format($log['amount'], 2) ?></div>
                                        </td>
                                        <td style="padding: 1rem 1.5rem;">
                                            <span style="display: inline-block; padding: 0.2rem 0.6rem; border-radius: 4px; font-size: 0.65rem; font-weight: 800; text-transform: uppercase; background: <?= $statusBg ?>; color: <?= $statusColor ?>;">
                                                <?= $log['status'] ?>
                                            </span>
                                        </td>
                                    </tr>
