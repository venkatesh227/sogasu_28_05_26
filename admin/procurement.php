...
$issueQuery = mysqli_query($conn, "
    ...
    LEFT JOIN orders
    ON orders.id = stock_issuance.order_id
    ...
    <div style="padding: 1rem;">
        ...
        ">
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem; gap: 1rem; flex-wrap: wrap;">
                ...
                <form method="GET" action="procurement.php" style="display:inline-flex; align-items:center; gap:8px; margin:0;">
                    <label for="categoryFilter" style="font-size:14px; font-weight:600; color:#475569;">Filter Category:</label>
                    <select name="category" id="categoryFilter" onchange="this.form.submit()" style="height:38px; border:1px solid #dbe2ea; border-radius:8px; padding:0 12px; font-size:14px; background:#fff; outline:none; min-width: 160px; cursor: pointer; color: #1e293b; font-weight: 500;">
                ...
                <tbody>
                  ...
                  <?php while($row = mysqli_fetch_assoc($procurementQuery)) : ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span style="background: #f1f5f9; color: #475569; padding: 4px 10px; border-radius: 20px; font-size: 12px; font-weight: 600;">
...
<?= htmlspecialchars($issue['quantity_issued']) ?>
                                ...
                                <td>
                                    <?= !empty($issue['order_code']) ? '<a href="view-order.php?id=' . intval($issue['order_id']) . '" style="font-weight:700; color:#4f46e5; text-decoration:none;"><i class="ri-external-link-line" style="vertical-align:middle; font-size:14px; margin-right:2px;"></i>#' . htmlspecialchars($issue['order_code']) . '</a>' : '<span style="color:#94a3b8;">N/A</span>' ?>
                                </td>
