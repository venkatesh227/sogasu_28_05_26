...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($errors['phone_no']) && !empty($old['phone_no'])) {
        foreach ($pdo->query("SELECT id, phone_no, alternate_no FROM suppliers WHERE is_deleted = 0") as $row) {
            ...
            if (
                $old['phone_no'] === $row['phone_no'] ||
                $old['phone_no'] === $row['alternate_no']
            ) {
    ...
    if (!empty($old['alternate_no'])) {
        foreach ($pdo->query("SELECT id, phone_no, alternate_no FROM suppliers WHERE is_deleted = 0") as $row) {
            ...
            if (
                $old['alternate_no'] === $row['phone_no'] ||
                $old['alternate_no'] === $row['alternate_no']
            ) {
                $errors['alternate_no'] = "Alternate number already exists";
                break;
            }
        }
    }
    ...
    if (!isset($errors['account_no']) && !empty($old['account_no'])) {
        foreach ($pdo->query("SELECT id, account_no FROM suppliers WHERE is_deleted = 0") as $row) {

            if ($id && $row['id'] == $id) {
                continue;
            }

            if ($old['account_no'] === $row['account_no']) {
    ...
    if (!isset($errors['ifsc_code']) && !empty($old['ifsc_code'])) {
        foreach ($pdo->query("SELECT id, ifsc_code FROM suppliers WHERE is_deleted = 0") as $row) {

            if ($id && $row['id'] == $id) {
                continue;
            }

            if (strcasecmp($old['ifsc_code'], $row['ifsc_code']) == 0) {
                $errors['ifsc_code'] = "IFSC code already exists";
                break;
            }
        }
    }
        ...
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <div>
                <h2 style="font-size: 1.5rem; font-weight: 700; color: #1e293b; margin: 0;">
                    <?= $id ? 'Edit' : 'Add New' ?> Supplier
                </h2>
                <p style="color: #64748b; margin-top: 0.25rem;">
                    <?= $id ? 'Update existing supplier details and bank configuration' : 'Register a new stock supplier with firm and bank info' ?>
                </p>
            </div>
            <button type="button" class="btn" onclick="history.back()"
                style="background: white; border: 1px solid #e2e8f0; color: #64748b; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-weight: 600; display: inline-flex; align-items: center; gap: 0.25rem;">
    ...
    <form method="POST" novalidate style="display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem;">
        ...
        <div style="display: flex; flex-direction: column; gap: 1.25rem;">
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                <h3
                    style="font-size: 1.1rem; font-weight: 600; color: #1e293b; margin-bottom: 1.25rem; display: flex; align-items: center; gap: 0.5rem;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                    <div class="form-group">
                        ...
                        <input type="text" name="supplier_name" class="form-control" placeholder="e.g. RK Textiles"
                            maxlength="100" value="<?= htmlspecialchars($old['supplier_name'] ?? '') ?>">
                        <?php if (isset($errors['supplier_name'])): ?>
                            <small
                                style="color:red; margin-top: 0.25rem; display: block;"><?= $errors['supplier_name'] ?></small>
                        <?php endif; ?>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Firm Name</label>
                        <input type="text" name="firm_name" class="form-control"
                            placeholder="e.g. RK Textiles Private Limited" maxlength="150"
                            value="<?= htmlspecialchars($old['firm_name'] ?? '') ?>">
                        <?php if (isset($errors['firm_name'])): ?>
                            <small
                                style="color:red; margin-top: 0.25rem; display: block;"><?= $errors['firm_name'] ?></small>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                    <div class="form-group">
                        ...
                        <input type="text" name="contact_person" class="form-control" placeholder="e.g. Rajesh Kumar"
                            maxlength="100" value="<?= htmlspecialchars($old['contact_person'] ?? '') ?>">
                        <?php if (isset($errors['contact_person'])): ?>
                            <small
                                style="color:red; margin-top: 0.25rem; display: block;"><?= $errors['contact_person'] ?></small>
                        <?php endif; ?>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Phone No</label>
                        <input type="text" name="phone_no" class="form-control" placeholder="Phone Number"
                            maxlength="10" value="<?= htmlspecialchars($old['phone_no'] ?? '') ?>">
                        <?php if (isset($errors['phone_no'])): ?>
                            <small
                                style="color:red; margin-top: 0.25rem; display: block;"><?= $errors['phone_no'] ?></small>
                        <?php endif; ?>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Alternate No</label>
                        <input type="text" name="alternate_no" class="form-control" placeholder="Alternate Number"
                            maxlength="10" value="<?= htmlspecialchars($old['alternate_no'] ?? '') ?>">
                        <?php if (isset($errors['alternate_no'])): ?>
                            <small style="color:red; margin-top:0.25rem; display:block;">
                                <?= $errors['alternate_no'] ?>
                            </small>
                        <?php endif; ?>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" name="gst_no" class="form-control" placeholder="GSTIN Format" maxlength="20"
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                    <div class="form-group">
                        ...
                        <input type="text" name="bank_name" class="form-control" placeholder="e.g. State Bank of India"
                            maxlength="100" value="<?= htmlspecialchars($old['bank_name'] ?? '') ?>">
                        <?php if (isset($errors['bank_name'])): ?>
                            <small
                                style="color:red; margin-top: 0.25rem; display: block;"><?= $errors['bank_name'] ?></small>
                        <?php endif; ?>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Account No</label>
                        <input type="text" name="account_no" class="form-control" placeholder="Bank Account Number"
                            maxlength="50" value="<?= htmlspecialchars($old['account_no'] ?? '') ?>">
                        <?php if (isset($errors['account_no'])): ?>
                            <small
                                style="color:red; margin-top: 0.25rem; display: block;"><?= $errors['account_no'] ?></small>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        ...
                        <input type="text" name="ifsc_code" class="form-control" placeholder="11-digit IFSC"
                            maxlength="20" value="<?= htmlspecialchars($old['ifsc_code'] ?? '') ?>">
                        <?php if (isset($errors['ifsc_code'])): ?>
                            <small
                                style="color:red; margin-top: 0.25rem; display: block;"><?= $errors['ifsc_code'] ?></small>
                        <?php endif; ?>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Branch</label>
                        <input type="text" name="bank_branch" class="form-control" placeholder="Bank Branch Name"
                            maxlength="100" value="<?= htmlspecialchars($old['bank_branch'] ?? '') ?>">
                        <?php if (isset($errors['bank_branch'])): ?>
                            <small
                                style="color:red; margin-top: 0.25rem; display: block;"><?= $errors['bank_branch'] ?></small>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 1.25rem;">
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                <h3 style="font-size: 1.1rem; font-weight: 600; color: #1e293b; margin-bottom: 1.25rem;">Configuration
                </h3>

                <div class="form-group">
                    <label class="form-label" style="font-weight: 600; color: #334155;">Status</label>
                    <select name="status" class="form-select"
                        style="padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 0.95rem; width: 100%;">
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                ...
                <button type="submit" class="btn btn-primary"
                    style="background: #4f46e5; border: none; padding: 12px; border-radius: 6px; font-weight: 600; color: white; width: 100%; cursor: pointer; margin-bottom: 0.75rem; display: flex; align-items: center; justify-content: center; gap: 0.25rem;">
                    <i class="ri-save-3-line"></i> <?= $id ? 'Update Supplier' : 'Save Supplier' ?>
                </button>
                <button type="button" class="btn" onclick="window.location.href='suppliers.php'"
                    style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 12px; border-radius: 6px; font-weight: 600; color: #64748b; width: 100%; cursor: pointer;">
                    Cancel
                </button>
            </div>
        </div>
