...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $errors = [];
    if (empty($_POST['customer_id'])) $errors['customer_id'] = "Customer is required";
    ...
    if (empty($_POST['due_date'])) $errors['due_date'] = "Due Date is required";
    if (empty($_POST['mobile_number'])) $errors['mobile_number'] = "Mobile Number is required";


if (empty($_POST['delivery_address'])) $errors['delivery_address'] = "Delivery Address is required";

if (empty($_POST['design_notes'])) $errors['design_notes'] = "Design Description / Remarks is required";
if (empty($_POST['fabric_details'])) $errors['fabric_details'] = "Fabric Details is required";

if (empty($_POST['base_price'])) $errors['base_price'] = "Base Price is required";
...
if (
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO orders (
                    order_code, customer_id, family_member_id, category_id, sub_category_id, 
                    fabric_details, notes, order_status, supervisor_id, 
                    base_price, extra_charges, total_amount, advance_paid, advance_payment_mode, due_date, measurement_unit
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");

            $stmt->execute([
                $order_code, $customer_id, $family_member_id, $category_id, $sub_category_id,
                $fabric_details, $design_notes, $order_status, $supervisor_id,
                $base_price, $extra_charges, $total_amount, $advance_paid, $advance_payment_mode, $due_date, $measurement_unit
            ]);
...
<main class="main-content">
    ...
    <form method="POST" novalidate enctype="multipart/form-data" style="display: grid; grid-template-columns: 2fr 1fr; gap: 1rem;">
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem;">
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                    <div class="form-group" style="position: relative;">
                        ...
                        <div style="position: relative;">
                            <input type="text" class="form-control" placeholder="Search name..." id="customer-search" autocomplete="off" oninput="showSuggestions('name')">
                            <i class="ri-search-line" style="position: absolute; right: 1rem; top: 50%; transform: translateY(-50%); color: #94a3b8;"></i>
                            <div id="customer-suggestions" class="suggestions-dropdown" style="display: none;"></div>
                        </div>
                        <?php if (isset($errors['customer_id'])) echo "<div style='color:red; font-size:0.9rem; margin-top:0.5rem;'>{$errors['customer_id']}</div>"; ?>
                    </div>
                    <div class="form-group" style="position: relative;">
                        <label class="form-label">Mobile Number</label>
                        <div style="position: relative;">
                            <input type="hidden" name="customer_id" id="customer-id-input">
                            <input type="tel" name="mobile_number" class="form-control" id="customer-mobile" placeholder="Search phone..." autocomplete="off" oninput="showSuggestions('phone')">
                            <?php if (isset($errors['mobile_number'])) echo "<div style='color:red;font-size:0.85rem;margin-top:5px'>{$errors['mobile_number']}</div>"; ?>
                            <i class="ri-search-line" style="position: absolute; right: 1rem; top: 50%; transform: translateY(-50%); color: #94a3b8;"></i>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1rem;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="delivery_address" class="form-control" id="customer-address" placeholder="Address...">
                        <?php if (isset($errors['delivery_address'])) echo "<div style='color:red;font-size:0.85rem;margin-top:5px'>{$errors['delivery_address']}</div>"; ?>
                    </div>
                </div>
            </div>
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                    <div class="form-group">
                        ...
                        <select class="form-select" id="order-cat-select" name="category_id" onchange="updateOrderSubCats()">
                            <option value="">Select Category</option>
                            <?php foreach ($categories as $cat): ?>
                                <option value="<?= $cat['id'] ?>"><?= htmlspecialchars($cat['category_name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                        <?php if (isset($errors['category_id'])) echo "<div style='color:red; font-size:0.9rem; margin-top:0.5rem;'>{$errors['category_id']}</div>"; ?>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Sub Category</label>
                        <select class="form-select" id="order-sub-cat-select" name="sub_category_id" onchange="loadMeasurements()">
                            <option value="">Select Sub Category</option>
                        </select>
                        <?php if (isset($errors['sub_category_id'])) echo "<div style='color:red; font-size:0.9rem; margin-top:0.5rem;'>{$errors['sub_category_id']}</div>"; ?>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        ...
                        <input type="text" name="fabric_details" class="form-control" placeholder="Source & Color (e.g. Saree Blouse, Red)">
                        <?php if (isset($errors['fabric_details'])) echo "<div style='color:red;font-size:0.85rem;margin-top:5px'>{$errors['fabric_details']}</div>"; ?>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Due Date</label>
                        <input type="date" name="due_date" class="form-control" id="due-date">
                        <?php if (isset($errors['due_date'])) echo "<div style='color:red; font-size:0.9rem; margin-top:0.5rem;'>{$errors['due_date']}</div>"; ?>
                    </div>
                </div>
            </div>
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                <h3 style="font-size: 1.1rem; font-weight: 600; color: #1e293b; margin-bottom: 1rem; display: flex; align-items: center; justify-content: space-between; gap: 0.5rem;">
                    <span style="display: flex; align-items: center; gap: 0.5rem;">
                        <i class="ri-ruler-line" style="color: var(--primary);"></i> Measurements
                    </span>
                    <div style="display: flex; background: #f1f5f9; padding: 2px; border-radius: 6px; font-size: 0.75rem; color: #64748b;">
                        <label style="margin: 0; padding: 4px 10px; cursor: pointer; border-radius: 4px; font-weight: 600; transition: all 0.2s;" id="unit-cms-label" class="<?= $default_unit === 'CMS' ? 'unit-active' : '' ?>">
                        ...
                        <label style="margin: 0; padding: 4px 10px; cursor: pointer; border-radius: 4px; font-weight: 600; transition: all 0.2s;" id="unit-inch-label" class="<?= $default_unit === 'INCH' ? 'unit-active' : '' ?>">
                ...
                <!-- Empty State -->
                <div id="measurements-empty" style="text-align: center; padding: 2rem; color: #94a3b8; border: 1px dashed #cbd5e1; border-radius: 6px;">
                ...
                <!-- Fields Container -->
                <div id="measurements-grid" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem;"></div>
                <?php if (isset($errors['measurements'])) echo "<div style='color:red;font-size:0.85rem;margin-top:10px'>{$errors['measurements']}</div>"; ?>
            </div>

            <!-- Photos & References -->
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px; margin-top: 1rem;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
                    ...
                    <div>
                        ...
                        <p style="font-size: 0.75rem; color: #64748b; margin-bottom: 0.75rem;">Upload images of the fabric to be stitched.</p>
                        <div class="upload-area" onclick="document.getElementById('fabric_photos').click()">
                            <i class="ri-upload-cloud-line"></i>
                            <span>Click to upload fabrics</span>
                            <input type="file" name="fabric_photos[]" id="fabric_photos" multiple accept="image/*" style="display: none;" onchange="previewImages(this, 'fabric-preview')">
                        </div>
                    ...
                    <div>
                        ...
                        <p style="font-size: 0.75rem; color: #64748b; margin-bottom: 0.75rem;">Upload sample designs or reference photos.</p>
                        <div class="upload-area" onclick="document.getElementById('sample_photos').click()" style="border-color: #e2e8f0;">
                            <i class="ri-image-add-line" style="color: #64748b;"></i>
                            <span>Click to upload samples</span>
                            <input type="file" name="sample_photos[]" id="sample_photos" multiple accept="image/*" style="display: none;" onchange="previewImages(this, 'sample-preview')">
                        </div>
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                <h3 style="font-size: 1.1rem; font-weight: 600; color: #1e293b; margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem;">
                    <i class="ri-clipboard-line" style="color: var(--primary);"></i> Notes & Design
                </h3>
                <div class="form-group">
                    <label class="form-label">Design Description / Remarks</label>
                    <textarea class="form-control" name="design_notes" id="design-notes" rows="3" placeholder="Neck depth, embroidery details, piping etc."></textarea>
                    <?php if (isset($errors['design_notes'])) echo "<div style='color:red;font-size:0.85rem;margin-top:5px'>{$errors['design_notes']}</div>"; ?>
                </div>
                <!-- Predefined Notes -->
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                ...
                <p style="font-size: 0.8rem; color: #64748b; margin-bottom: 1rem;">Select additional services to include in this order.</p>
                
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 1rem;">
                    <?php foreach ($standalone_services as $srv): ?>
                        <label class="service-checkbox-card" style="display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; border: 1px solid #e2e8f0; border-radius: 8px; cursor: pointer; transition: all 0.2s;" title="<?= htmlspecialchars($srv['description'] ?? '') ?>">
                            <input type="checkbox" name="selected_services[]" value="<?= $srv['id'] ?>" data-price="<?= $srv['base_price'] ?>" onchange="calculateTotal()" style="width: 18px; height: 18px; cursor: pointer;">
                            <div style="flex: 1;">
                                <div style="font-size: 0.9rem; font-weight: 600; color: #1e293b;"><?= htmlspecialchars($srv['service_name']) ?></div>
                                <div style="font-size: 0.8rem; color: #64748b;">₹ <?= number_format($srv['base_price'], 2) ?></div>
                            </div>
                        </label>
                    <?php endforeach; ?>
                </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem;">
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                ...
                <div style="display: flex; justify-content: space-between; margin-bottom: 1.5rem; font-size: 0.9rem; color: #64748b;">
                    <span>Order ID</span>
                ...
                <div class="form-group" style="margin-bottom: 1rem;">
                    ...
                    <div style="position: relative;">
                        ...
                        <input type="number" step="0.01" name="base_price" class="form-control" id="base_price" style="padding-left: 2rem;" placeholder="0.00" oninput="calculateTotal()">
                        <?php if (isset($errors['base_price'])) echo "<div style='color:red;font-size:0.85rem;margin-top:5px'>{$errors['base_price']}</div>"; ?>
                    </div>
                </div>
                <div class="form-group" style="margin-bottom: 1rem;">
                    ...
                    <div style="position: relative;">
                        <span style="position: absolute; left: 1rem; top: 50%; transform: translateY(-50%); color: #64748b;">₹</span>
                        <input type="number" step="0.01" name="extra_charges" class="form-control" id="extra_charges" style="padding-left: 2rem;" placeholder="0.00" oninput="calculateTotal()">
                    </div>
                </div>
                <div class="form-group" style="margin-bottom: 1rem;">
                ...
                <div class="form-group">
                    ...
                    <div style="position: relative;">
                        <span style="position: absolute; left: 1rem; top: 50%; transform: translateY(-50%); color: #64748b;">₹</span>
                        <input type="number" name="advance_paid" class="form-control" style="padding-left: 2rem;" placeholder="0.00">
                    </div>
                </div>
            ...
            <div style="background: white; border: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 8px;">
                ...
                <button type="submit" name="submit_order" value="1" class="btn btn-primary w-full" style="justify-content: center; width: 100%; margin-bottom: 1rem;">Create Order</button>
                <button type="button" class="btn w-full" style="justify-content: center; width: 100%; background: #f8fafc; border: 1px solid #e2e8f0; color: #64748b;">Save as Draft</button>
            </div>
