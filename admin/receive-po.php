        ...
        ");
            ...
            for ($i = 0; $i < $items_count; $i++) {
                ...
                if ($new_received_qty > $poi['quantity']) {
                    throw new Exception(
                        "Received quantity cannot exceed ordered quantity for item: " . $poi['item_name']
                    );
                }
            ...
            $po_total = floatval($total_stmt->fetchColumn());

            $update_po = $pdo->prepare("
                UPDATE purchase_orders
                SET status = ?, total_amount = ?, invoice_no = ?, invoice_date = ?, invoice_file = ?
                WHERE id = ?
            ");
...
<main class="main-content">
    ...
    <div>
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem;">
            <div>
                <h2 style="font-size: 1.5rem; font-weight: 700; color: #1e293b; margin: 0;">Receive Stock:
                    <?= htmlspecialchars($po['po_number']) ?>
                </h2>
                <p class="text-muted" style="margin: 0.25rem 0 0 0;">Record actual received quantities and link the
                    supplier invoice</p>
            </div>
    ...
    <?php if (!empty($errors)): ?>
        <div
            style="background: rgba(239, 68, 68, 0.08); border: 1px solid #fca5a5; border-radius: 8px; padding: 1rem; color: #b91c1c; font-weight: 500; font-size: 0.9rem; margin-bottom: 1.5rem; line-height: 1.5;">
            <div style="font-weight: 700; margin-bottom: 0.5rem;"><i class="ri-error-warning-line"></i> Please fix the
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1.25rem;">
                <div class="form-group">
                    ...
                    <input type="text" name="invoice_no" class="form-control" required placeholder="e.g. INV-2026-9041"
                        value="<?= isset($_POST['invoice_no']) ? htmlspecialchars($_POST['invoice_no']) : '' ?>">
                </div>
                <div class="form-group">
                    <label class="form-label">Invoice Date <span style="color: red;">*</span></label>
                    <input type="date" name="invoice_date" class="form-control" required
                        value="<?= isset($_POST['invoice_date']) ? htmlspecialchars($_POST['invoice_date']) : date('Y-m-d') ?>">
                </div>
                <div class="form-group">
                    <label class="form-label">Supplier Name</label>
                    <input type="text" class="form-control" readonly
                        style="background: #f8fafc; font-weight: 700; color: #334155;"
                ...
                <div class="form-group">
                    ...
                    <div id="filePreview"
                        style="font-size: 0.8rem; color: #64748b; margin-top: 0.4rem; font-weight: 500;">No file chosen
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="table" style="min-width: 900px; margin-bottom: 0;">
                    <thead>
                        <tr style="border-bottom: 2px solid #e2e8f0;">
                            ...
                            <th style="width: 18%; text-align: right;">Received Qty <span style="color: red;">*</span>
                            </th>
                            <th style="width: 18%; text-align: right;">Unit Price (₹) <span style="color: red;">*</span>
                            </th>
                        ...
                        <?php foreach ($items as $index => $item): ?>
                            <tr style="border-bottom: 1px solid #f1f5f9;">
                                <td>
                                    ...
                                    <div style="font-size: 0.75rem; color: #64748b; font-weight: 500;">SKU:
                                        <?= htmlspecialchars($item['sku'] ?: 'Auto-generated') ?>
                                    </div>
                                    <input type="hidden" name="item_id[]" value="<?= $item['id'] ?>">
                                </td>
                                <td>
                                    <span
                                        style="font-weight: 600; color: #475569; background: #f1f5f9; padding: 4px 10px; border-radius: 8px; font-size: 0.8rem;">
                                ...
                                <td>
                                    <input type="number" name="item_cost[]" class="form-control" step="0.01" min="0"
                                        ...
                                        value="<?= isset($_POST['item_cost'][$index]) ? htmlspecialchars($_POST['item_cost'][$index]) : htmlspecialchars($item['cost']) ?>">
                                </td>
