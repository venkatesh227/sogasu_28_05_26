    ...
    if ($action === 'change_password') {
        ...
        if (strlen($new_password) < 6) {
            ...
            if ($updateStmt->execute([$hashed, $user_id])) {
                $pwd_success = $t['password_changed'];
            }
        }
    }
...
<div class="container">
    ...
    <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 1rem; border-radius: 8px; margin-bottom: 1rem;">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div style="display: flex; gap: 1rem; align-items: center;">
                ...
                <div>
                    ...
                    <div style="font-size: 0.85rem; color: var(--text-muted);"><?php echo $notifCount; ?> <?php echo $t['new_updates']; ?></div>
                </div>
            </div>
            <button onclick="toggleNotifications()" style="background: transparent; border: none; color: var(--text-muted); cursor: pointer;">
        ...
        <div id="notif-list" style="display: none; margin-top: 1rem; max-height: 200px; overflow-y: auto;">
            ...
            <div style="padding: 0.5rem; border-bottom: 1px solid rgba(245, 158, 11, 0.2); font-size: 0.9rem;">
                ...
                <p style="margin-top: 0.25rem; color: var(--text-muted);"><?php echo htmlspecialchars($notif['message'] ?? ''); ?></p>
    ...
    <div style="background: #d1fae5; border-left: 4px solid #10b981; padding: 1rem; border-radius: 8px; margin-bottom: 1rem; color: #065f46;">
        <?php echo $success; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="card" style="text-align: center; padding: 2rem 1rem; border: none; background: linear-gradient(135deg, var(--background), #fff);">
        <div style="position: relative; display: inline-block; margin-bottom: 1rem;">
            ...
            <button onclick="document.getElementById('photoInput').click()" style="position: absolute; bottom: 0; right: 0; background: var(--primary); color: white; border: none; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 2px solid white; cursor: pointer;">
        ...
        <p style="color: var(--text-muted); font-size: 0.95rem;">
            <?php echo htmlspecialchars($employee['job_role']); ?> • <?php echo $t['job_role_id']; ?> <?php echo htmlspecialchars($employee['id']); ?>
        </p>
    </div>
    ...
    <div class="card" style="padding: 0;">
        <div style="padding: 1rem; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center;">
            ...
            <form method="POST" style="display: inline;">
                ...
                <button type="submit" class="toggle-switch <?php echo ($employee['notification_enabled'] ?? 1) ? 'active' : ''; ?>" style="border: none; cursor: pointer;">
                    <div class="knob"></div>
                </button>
            </form>
        </div>
        <div style="padding: 1rem; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center;">
            <div style="display: flex; gap: 1rem; align-items: center;">
                <i class="ri-translate" style="color: var(--text-muted); font-size: 1.25rem;"></i>
                <span style="font-size: 0.95rem; font-weight: 500;"><?php echo $t['language']; ?></span>
            </div>
            <button onclick="openLanguageModal()" style="background: transparent; border: none; color: var(--text-muted); display: flex; align-items: center; gap: 0.5rem; cursor: pointer; font-size: 0.9rem;">
                <span><?php echo $language === 'hi' ? 'Hindi' : ($language === 'te' ? 'Telugu' : 'English'); ?></span>
        ...
        <div style="padding: 1rem; display: flex; justify-content: space-between; align-items: center;">
            ...
            <button onclick="openPasswordModal()" style="background: transparent; border: none; color: var(--text-muted); cursor: pointer;">
                <i class="ri-arrow-right-s-line"></i>
            </button>
        </div>
    </div>
...
<div id="editModal" class="modal" style="display: none;">
    <div class="modal-content">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
            ...
            <button onclick="closeModal('editModal')" style="background: transparent; border: none; font-size: 1.5rem; cursor: pointer;">×</button>
        </div>
        <form method="POST">
            <input type="hidden" name="action" value="update_profile">
            <div style="margin-bottom: 1rem;">
                <label style="display: block; font-size: 0.9rem; font-weight: 600; margin-bottom: 0.5rem;"><?php echo $t['phone_number']; ?></label>
                <input type="tel" name="phone" id="editPhone" value="<?php echo htmlspecialchars($employee['phone'] ?? ''); ?>" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border); border-radius: 8px; font-family: inherit;">
            </div>
            <div style="margin-bottom: 1rem;">
                <label style="display: block; font-size: 0.9rem; font-weight: 600; margin-bottom: 0.5rem;"><?php echo $t['address']; ?></label>
                <textarea name="address" id="editAddress" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border); border-radius: 8px; font-family: inherit; height: 80px; resize: vertical;"><?php echo htmlspecialchars($employee['address'] ?? ''); ?></textarea>
            </div>
            <button type="submit" style="width: 100%; background: var(--primary); color: white; border: none; padding: 0.75rem; border-radius: 8px; font-weight: 600; cursor: pointer;"><?php echo $t['save_changes']; ?></button>
        </form>
    </div>
</div>
<?php endif; ?>
...
<div id="passwordModal" class="modal" style="display: none;">
    <div class="modal-content">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
            ...
            <button onclick="closeModal('passwordModal')" style="background: transparent; border: none; font-size: 1.5rem; cursor: pointer;">×</button>
        </div>
        <?php if (isset($pwd_error)): ?>
        <div style="background: #fee2e2; color: #991b1b; padding: 0.75rem; border-radius: 8px; margin-bottom: 1rem; font-size: 0.9rem;">
            <?php echo $pwd_error; ?>
        </div>
        <?php endif; ?>
        <?php if (isset($pwd_success)): ?>
        <div style="background: #d1fae5; color: #065f46; padding: 0.75rem; border-radius: 8px; margin-bottom: 1rem; font-size: 0.9rem;">
        ...
        <form method="POST">
            ...
            <div style="margin-bottom: 1rem;">
                <label style="display: block; font-size: 0.9rem; font-weight: 600; margin-bottom: 0.5rem;"><?php echo $t['employee_name']; ?></label>
                <input type="text" value="<?php echo htmlspecialchars($employee['first_name'] . ' ' . $employee['last_name']); ?>" disabled style="width: 100%; padding: 0.75rem; border: 1px solid var(--border); border-radius: 8px; font-family: inherit; background: var(--background);">
            </div>
            <div style="margin-bottom: 1rem;">
                <label style="display: block; font-size: 0.9rem; font-weight: 600; margin-bottom: 0.5rem;"><?php echo $t['new_password']; ?></label>
                <input type="password" name="new_password" required style="width: 100%; padding: 0.75rem; border: 1px solid var(--border); border-radius: 8px; font-family: inherit;">
            </div>
            <div style="margin-bottom: 1rem;">
                <label style="display: block; font-size: 0.9rem; font-weight: 600; margin-bottom: 0.5rem;"><?php echo $t['confirm_password']; ?></label>
                <input type="password" name="confirm_password" required style="width: 100%; padding: 0.75rem; border: 1px solid var(--border); border-radius: 8px; font-family: inherit;">
            </div>
            <button type="submit" style="width: 100%; background: var(--primary); color: white; border: none; padding: 0.75rem; border-radius: 8px; font-weight: 600; cursor: pointer;"><?php echo $t['change_password']; ?></button>
        </form>
    </div>
</div>
...
<div id="languageModal" class="modal" style="display: none;">
    <div class="modal-content">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
            ...
            <button onclick="closeModal('languageModal')" style="background: transparent; border: none; font-size: 1.5rem; cursor: pointer;">×</button>
        </div>
        <div id="languageOptions">
            ...
            ?>
            <label style="display: flex; align-items: center; padding: 0.75rem; border: 1px solid var(--border); border-radius: 8px; margin-bottom: 0.5rem; cursor: pointer;">
        ...
        <div id="languageMessage" style="display: none; background: #d1fae5; color: #065f46; padding: 0.75rem; border-radius: 8px; margin-top: 1rem; font-size: 0.9rem;">
