...
<div class="container">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
        ...
        <div style="font-size: 0.8rem; color: #64748b; font-weight: 600;"><?= date('M d', strtotime($week_dates[0])) ?>
            - <?= date('d, Y', strtotime($week_dates[6])) ?></div>
    </div>
                    ...
                    <div style="padding: 1rem; flex-grow: 1;">
                        <?php if ($shift): ?>
                            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                ...
                                <div
                                    style="background: <?= $shift['color'] ?>20; color: <?= $shift['color'] ?>; padding: 4px 10px; border-radius: 6px; font-size: 0.7rem; font-weight: 800;">
                                    <?= $shift['short_code'] ?>
                                </div>
                            </div>
                            <!-- Request Change Link -->
                            <div style="margin-top: 0.5rem; text-align: right;">
                                <?php if (in_array('appointments_create', $permissions)): ?>
                                    ...
                                    <button
                                        onclick="openRequestModal('<?= $date ?>', '<?= htmlspecialchars($shift['shift_name']) ?>')"
                                        style="background: none; border: none; color: #db2777;">
                        ...
                        <?php else: ?>
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <div style="color: #94a3b8; font-size: 0.85rem; font-style: italic;">No shift assigned / Day Off
                                </div>
                                <button onclick="openRequestModal('<?= $date ?>', 'None')"
                                    style="background: #f1f5f9; border: none; color: #475569; font-size: 0.65rem; font-weight: 700; padding: 4px 8px; border-radius: 4px; cursor: pointer;">
                                    Assign Shift
                                </button>
                            </div>
                        <?php endif; ?>
    ...
    <div id="requestModal"
        ...
        <div
            ...
            <form id="requestForm" onsubmit="event.preventDefault(); submitRequest()">
                ...
                <div style="margin-bottom: 1rem;">
                    <label
                        style="display: block; font-size: 0.85rem; font-weight: 600; color: #64748b; margin-bottom: 0.5rem;">Desired
                        Shift <span style="color: #ef4444;">*</span></label>
                    <select name="requested_shift_id" required
                        style="width: 100%; padding: 0.75rem; border: 1px solid #e2e8f0; border-radius: 12px; outline: none; background: #f8fafc; font-size: 0.95rem;">
                ...
                <div style="margin-bottom: 1.5rem;">
                    <label
                        style="display: block; font-size: 0.85rem; font-weight: 600; color: #64748b; margin-bottom: 0.5rem;">Reason
                        for Change</label>
                    <textarea name="reason" placeholder="e.g. Personal emergency, Transportation issues..." rows="3"
                        style="width: 100%; padding: 0.75rem; border: 1px solid #e2e8f0; border-radius: 12px; outline: none; resize: none; font-size: 0.95rem;"></textarea>
                </div>

                <button type="submit"
                    style="width: 100%; background: #db2777; color: white; border: none; padding: 1rem; border-radius: 16px; font-weight: 700; font-size: 1rem; box-shadow: 0 10px 15px -3px rgba(219, 39, 119, 0.3);">Submit
                    Request</button>
            </form>
        </div>
    </div>
