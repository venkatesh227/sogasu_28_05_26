...
if (isset($_POST['action'])) {
    ...
    if ($action === 'punch_in') {
        ...
        // No shift / Off shift check (existing functionality preserved)
        if (!$today_shift || strpos(strtolower($today_shift['name']), 'off') !== false) {
            echo json_encode([
                'success' => false,
                'error' => 'You do not have an active shift assigned for today.'
            ]);
            exit;
        }
        ...
        */
        ...
        if ($currentDateTime > $shiftEndDateTime) {
            echo json_encode([
                'success' => false,
                'error' => 'Punch-in not allowed. Shift ended at ' . date('h:i A', strtotime($today_shift['end_time'])) . '.'
            ]);
            exit;
        }
        $status = 'Present';
...
<div class="container">
    <div class="card" style="text-align: center; padding: 2rem 1rem;">
        ...
        <div id="current-time" style="font-size: 2.5rem; font-weight: 800; color: #1e293b; margin-bottom: 0.5rem;">
            00:00:00</div>

        <?php if ($today_shift): ?>
            <div
                style="margin-bottom: 2rem; display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.25rem 0.75rem; background: <?= $today_shift['color'] ?>15; color: <?= $today_shift['color'] ?>; border-radius: 20px; font-size: 0.75rem; font-weight: 700;">
                <i class="ri-time-line"></i> <?= htmlspecialchars($today_shift['name']) ?>:
                <?= date('h:i A', strtotime($today_shift['start_time'])) ?> -
                <?= date('h:i A', strtotime($today_shift['end_time'])) ?>
            </div>
        <?php else: ?>
            <div
                style="margin-bottom: 2rem; display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.25rem 0.75rem; background: #fee2e2; color: #ef4444; border-radius: 20px; font-size: 0.75rem; font-weight: 700;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
            <div style="background: #f8fafc; padding: 1rem; border-radius: 12px; border: 1px solid #e2e8f0;">
                <div style="font-size: 0.7rem; color: #94a3b8; text-transform: uppercase; margin-bottom: 0.25rem;">Check
                    In</div>
                <div style="font-weight: 700; color: #1e293b; font-size: 1.1rem;">
                    <?= (!empty($today_att) && $today_att['check_in']) ? date('h:i A', strtotime($today_att['check_in'])) : '--:--' ?>
                </div>
            </div>
            <div style="background: #f8fafc; padding: 1rem; border-radius: 12px; border: 1px solid #e2e8f0;">
                <div style="font-size: 0.7rem; color: #94a3b8; text-transform: uppercase; margin-bottom: 0.25rem;">Check
                    Out</div>
                <div style="font-weight: 700; color: #1e293b; font-size: 1.1rem;">
                    <?= (!empty($today_att) && $today_att['check_out']) ? date('h:i A', strtotime($today_att['check_out'])) : '--:--' ?>
                </div>
            </div>
        </div>
        <?php if (in_array('hr_create', $permissions)): ?>
            ...
            if (!$last_log || $last_log === 'Out'): ?>
                <button
                    onclick="<?= $canPunchIn ? "punch('punch_in')" : "Swal.fire('No Shift', 'You cannot punch in because you have no shift assigned today.', 'warning')" ?>"
                    class="punch-btn"
